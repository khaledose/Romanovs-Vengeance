...
namespace OpenRA.Mods.RA2.Widgets.Logic
{
	...
	public class ObserverStatsSPLogic : ChromeLogic
	{
		...
		public ObserverStatsSPLogic(World world, ModData modData, WorldRenderer worldRenderer, Widget widget, Dictionary<string, MiniYaml> logicArgs)
		{
			...
			MiniYaml yaml;
			var keyNames = Enum.GetNames(typeof(ObserverStatsSPPanel));
			...
			if (ShowScrollBar)
			{
				...
				AdjustHeader(minimalStatsHeaders);
		...
		void ClearStats()
		{
			...
			minimalStatsHeaders.Visible = false;
		...
		ScrollItemWidget MinimalStats(Player player)
		{
			minimalStatsHeaders.Visible = true;
			var template = SetupPlayerScrollItemWidget(minimalPlayerTemplate, player);
