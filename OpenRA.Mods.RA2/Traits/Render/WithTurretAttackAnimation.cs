...
namespace OpenRA.Mods.RA2.Traits.Render
{
	...
	public class WithTurretAttackAnimation : ConditionalTrait<WithTurretAttackAnimationInfo>, ITick, INotifyAttack
	{
		...
		{
			if (!IsTraitDisabled && a.Info.Name == Info.Armament && Info.DelayRelativeTo == AttackDelayType.Attack)
				NotifyAttack(self);
		}

		void INotifyAttack.PreparingAttack(Actor self, in Target target, Armament a, Barrel barrel)
		{
			if (!IsTraitDisabled && a.Info.Name == Info.Armament && Info.DelayRelativeTo == AttackDelayType.Preparation)
				NotifyAttack(self);
		}
