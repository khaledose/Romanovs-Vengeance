...
namespace OpenRA.Mods.RA2.Traits
{
	...
	public class SlaveMinerMaster : BaseSpawnerMaster, INotifyTransform, ITick, IIssueOrder, IResolveOrder
	{
		...
		public WPos? LastOrderLocation = null;
		private int respawnTicks = 0;
		private int kickTicks;
		private bool allowKicks = true; // allow kicks?
		private bool initialSlaveSpawn = true;
		...
		public void AfterTransform(Actor toActor)
		{
			...
			harvesterMaster.SetMasterTargetPosition(LastOrderLocation);
		...
		public void ResolveOrder(Actor self, Order order)
		{
			...
			{
				LastOrderLocation = order.Target.CenterPosition;
				Transform();
			}
		...
		void ITick.Tick(Actor self)
		{
			...
			if (respawnTicks <= 0)
			{
				...
				var deadSlave = SlaveEntries.FirstOrDefault(s => !s.IsValid);

				if (deadSlave is null)
				{
					return;
				}

				Replenish(self, deadSlave);
				SpawnIntoWorld(self, deadSlave.Actor, self.CenterPosition);
			}
		}
	}
}
